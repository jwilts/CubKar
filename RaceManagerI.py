...
##	Version: 3
##  Date: Oct 24 /2021
##  - Add variables for small LCD to settings file
##  - switch time to use monotonic which is more reliable
##  - Add code for LED Bank (Specific place, specific bank)
##  - NOTE: If you turn on LCD then you cannot use PINS 3, 5 (LEDS 1 & 2 on PCB Board, switch to PINS 16/18)
##  - Added code to read database settings from the RMSettings.ini
##  - Remove 8x8 Matrix Code 
##
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
##	Version: 2.3
##  Date: Feb 24 /2020
...
if LoadLCD == "true":
    import I2C_LCD_driver  ## LCD Driver - New in Version 3.0
                           ## Conflicts with PCB LEDs 1 and 2
...
LcdYN = str(get_setting(path, 'Settings', 'LCD_yn', False))
LCDAddress = str(get_setting(path, 'Settings', 'LCD_Address', False))

############################################################################################################
## Database Information
############################################################################################################
## Loaded later when connection string required
...
############################################################################################################
def LightPlace(LightBank, Place) :
    if LightBank == 1:
        x = 0
        while x < Place:
            GPIO.setup(leds[x],GPIO.OUT)
...
def my_mainloop():
    ...
    while 1:
        ...
        while (time.monotonic() - StartTime) < MaxRaceTime and Finishers < 3:
            if GPIO.event_detected( LDRLaneOne ):
                ...
                if ( time.monotonic() - StartTime ) > ShortRaceTime:
                    if LDRLaneOneFinishTime == 0:		# Check and see if a time has been recorded for Lane 1 to finish.  (Way to deal with multiple events)
                        LDRLaneOneFinishTime = time.monotonic()
            ...
            if GPIO.event_detected( LDRLaneTwo ):
                ...
                if ( time.monotonic() - StartTime ) > ShortRaceTime:
                    if LDRLaneTwoFinishTime == 0:
                        LDRLaneTwoFinishTime = time.monotonic()
                        Finishers +=1
            ...
            if GPIO.event_detected( LDRLaneThree ):
                ...
                if ( time.monotonic() - StartTime ) > ShortRaceTime:
                    if LDRLaneThreeFinishTime == 0:
                        LDRLaneThreeFinishTime = time.monotonic()
                        Finishers +=1
...
try:
    ...
    # code to run before the program
