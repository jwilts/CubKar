...
RelayYN = str(get_setting(path, 'Settings', 'relay_yn', True))
...
## if display = true then print results, otherwise just return results 
    if 'Unit' in kwargs :
        # sql = "SELECT MAX(RaceCounter), MAX(Heat), MIN(RaceTime) from RaceResults WHERE Unit= '"+ kwargs['Unit'] + "';"
        sql = "SELECT MAX(A.RaceCounter), MIN(A.RaceTime), B.Heat FROM raceresults A INNER JOIN	trackheat B	ON A.Unit = B.TrackID WHERE A.Unit = '"+ kwargs['Unit'] + "' AND A.RaceTime > 00 ;"
    
    else :
        sql = "SELECT MAX(RaceCounter) from RaceResults ;"
    ## end if kwargs
...
def my_mainloop():
    ...
    if LoadRFPAD == "true" :
        ...
        details = {}
        RaceStats =  lookupRaceStats( "False", Unit)
        details = RaceStats[0]
